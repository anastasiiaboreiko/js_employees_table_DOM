...
form.addEventListener("submit", (e)=>{
    ...
    for (const [key, value] of formData){
        ...
        if (key === "salary") td.textContent = formatSalary(value);
        else td.textContent = capitalizeWords(value);
        tr.appendChild(td);
    }
    ...
    if (!latinRegex.test(nameInForm) || !latinRegex.test(positionInForm)) {
        pushNotification(100, 10, "Error", "Please, use only Latin letters.", "error");
        return;
    }
    if (nameInForm.length < 4) {
        pushNotification(100, 10, "Error", `Please lengthen your name to 4 characters or more (you are currently using ${nameInForm.length} character(s)).`, "error");
        return;
    }
    const age = Number(formData.get("age"));
    if (isNaN(age) || age < 18 || age > 90) {
        pushNotification(100, 10, "Error", "Age must be between 18 and 90", "error");
        return;
    }
    tbody.appendChild(tr);
    pushNotification(100, 10, "Success", `Great! The information has been added to the table.`, "success");
    form.reset();
});
...
function pushNotification(posTop, posRight, title, description, type) {
    ...
    if ([
        "success",
        "error"
    ].includes(type)) message.className = `notification ${type}`;
    else message.className = "notification";
